...
{#if isMobile}
  <button
    class="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-center gap-3 bg-white border-t border-gray-200 px-4 py-3 shadow-lg dark:border-gray-800 dark:bg-gray-900"
  ...
  <Modal bind:open={showCheckout} title="Warenkorb" class="max-w-full lg:max-w-md">
    <div class="space-y-4">
      ...
      {:else}
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-gray-800 space-y-2">
          ...
          <Button variant="success" class="w-full mt-2" onclick={() => showCheckout = false}>Weiter zur Zahlung</Button>
        </div>
      {/if}
    </div>
  </Modal>

  <Modal bind:open={showCheckout} title="Bezahlen" class="max-w-full lg:max-w-md">
    <div class="space-y-4">
